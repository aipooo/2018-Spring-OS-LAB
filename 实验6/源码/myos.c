...
void help()	/*提示信息*/ 
{
	...
	print("    #pro: Creat any number of process  e.g: run 23131\n\r"); 
	...
	print("    Others: int 21h , int 33h , int 34h , int 35h , int 36h , call4int\n\n\r");
}
...
void Random_Load()
{
	...
	for( i=3; i<strlen(input);i++ )
		if( input[i]!=' ' && (input[i]<'1' || input[i]>'5') )
		{
			Print("Error Input!");
			return;
		}
