...
PCB *get_current_process_PCB() {
	return &PCB_LIST[current_process];
}
...
void schedule() {
	...
	do {
		current_process++;
		if (current_process >= process_number)
			current_process = 1;
	} while (PCB_LIST[current_process].status == PCB_BLOCKED || PCB_LIST[current_process].status == PCB_EXIT);
	current_process_PCB_ptr = get_current_process_PCB();
...
void do_exit(int ss) {
	...
	PCB_LIST[current_process].status = PCB_EXIT;
...
semaphore semaphoreList[NRsem];

int semaGet(int value) {
    int i = 0;
    while (semaphoreList[i++].used && i < NRsem)
    	i++;
    if (i < NRsem) {
        semaphoreList[i].used = true;
        semaphoreList[i].count = value;
        semaphoreList[i].front = 0;
        semaphoreList[i].tail = 0;
		PCB_LIST[current_process].regs.ax = i;
		PCB_Restore();
		return i;
    }
	else {
		PCB_LIST[current_process].regs.ax = -1;
		PCB_Restore();
		return -1;
	}
}
...
void semaFree(int s) {
	semaphoreList[s].used = false;
}

void semaBlock(int s) {
	PCB_LIST[current_process].status = PCB_BLOCKED;
	if ((semaphoreList[s].tail + 1) % NRpcb == semaphoreList[s].front) {
		print("Kernal: Too many blocked processes!\r\n");
		return;
	}
	semaphoreList[s].blocked_pcb[semaphoreList[s].tail] = current_process;
	semaphoreList[s].tail = (semaphoreList[s].tail + 1) % NRpcb;
}

void semaWakeUp(int s) {
	int t;
	if (semaphoreList[s].tail == semaphoreList[s].front) {
		print("No blocked process to wake up!\r\n");
		return;
	}
	t = semaphoreList[s].blocked_pcb[semaphoreList[s].front];
	PCB_LIST[t].status = PCB_READY;
	semaphoreList[s].front = (semaphoreList[s].front + 1) % NRpcb;
}
...
void initsema() {
	int i;
	for (i = 0; i < NRsem; ++i) {
		semaphoreList[i].used = false;
		semaphoreList[i].count = 0;
		semaphoreList[i].front = 0;
		semaphoreList[i].tail = 0;
	}
}
